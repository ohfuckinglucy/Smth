...
struct TrieNode {
    char value;
    bool isEndOfWord;  // Индикатор конца слова
    TrieNode* children[26];  // Для каждой буквы алфавита
};
...
void trie_insert(Trie* trie, const string& key) {
    TrieNode* node = trie->root;
    for (char c : key) {
        int index = c - 'a';  // Допустим, что ключ состоит из строчных букв
...
void trie_print_helper(TrieNode* node, string prefix, int level) {
    if (node->isEndOfWord) {
        cout << prefix << "\n";
    }
    for (int i = 0; i < 26; i++) {
